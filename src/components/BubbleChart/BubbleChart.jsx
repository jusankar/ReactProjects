...
export default function BubbleChart({ data }) {
  ...
  const tooltipWidth = 200; // or whatever the width of your tooltip is
  const tooltipHeight = 100; // or whatever the height of your tooltip is
  ...
  return (
    <div className={s.mainConatiner}>
      <svg width={svgWidth} height={svgHeight} className={`${s.container} ${s.svgContainer}`}>
        {circles &&
          circles.map((circle) => {
            return (
              <g
              ...
              >
                ...
                {circle.data.title.length <= circle.r / 5 && (
                  <foreignObject
                    x={circle.x - circle.r}
                    y={circle.y - circle.r}
                    width={circle.r * 2}
                ...
                {hoveredCircle === circle.data.id && (
                  ...
                  >
                    <div className={s.tooltipContent}>
                      ...
                      <div>Salary</div>
                      <div className={s.toolTipCount}>${circle.data.count}</div>
                    </div>
                  </foreignObject>
                )}
              </g>
